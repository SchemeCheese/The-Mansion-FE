...
import { Col, DatePicker, Input, Row, Select, Skeleton, Space, Spin, Table, Tag } from 'ui/antd';
...
function ChannelManager({ fromDate, setFromDate }: Props) {
  ...
  useEffect(() => {
    if (updateRoomAvailableChanged('status', 'SUCCESS')) {
      notify.success(t('message.Update room available successfully!'));
  ...
  if (channelData.is_searching === false) {
    ...
    for (let index = 0; index < 10; index++) {
      dates.push(
        ...
        >
          <Skeleton.Button active block className={reservationListStyles.loadingDate} />
        </Col>,
      );
    }
  }
  ...
  return (
    <>
      ...
      <div className={reservationListStyles.channelPanel}>
        <Row className={reservationListStyles.channelHeaderRow}>
          <Col
            className={reservationListStyles.channelDatePickerCol}
            span={9}
            style={{
          ...
          >
            <Space size="large">
              ...
              <DatePicker
                aria-label="Channel manager date picker"
                ...
                format="D MMM Y"
                id="reservation-channel-manager-date-picker"
          ...
          <Col span={15}>
            <Row className={reservationListStyles.channelDateCells}>{dates}</Row>
          </Col>
        </Row>
        <Row className={reservationListStyles.channelFilterRow}>
          <Col span={24}>
            <Space className={reservationListStyles.channelFilterBar} size="middle">
            ...
            /> */}
              <p
                aria-hidden="true"
                className={reservationListStyles.channelClearFilters}
                onClick={() => {
                  setChannelSearch({
                    room_type: '',
                    channel: '',
        ...
        {channelData.is_searching ? (
          <Row className={reservationListStyles.channelLoadingRow}>
            <div style={{ textAlign: 'center', width: '100%' }}>
              <Spin style={{ paddingTop: 150 }} />
            </div>
          </Row>
        ) : (
          <Row className={reservationListStyles.channelResultsRow}>
            {channelData.channels
              ...
              .map((item: any) => {
                const columns = [
                  ...
                  },
                  {
                    ...
                    render: (value: any, record: object, index: number) => {
                      ...
                      return (
                        <span
                          style={{ color: 'rgba(0, 0, 0, 0.65)', fontSize: 12, fontWeight: 700 }}
                        >
                          RATES
                        </span>
                      );
                  ...
                  },
                  {
                    ...
                    className: reservationListStyles.channelDataCell,
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[0].day === 'Sun' ||
                              channelData.dates[0].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[1].day === 'Sun' ||
                              channelData.dates[1].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[2].day === 'Sun' ||
                              channelData.dates[2].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[3].day === 'Sun' ||
                              channelData.dates[3].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[4].day === 'Sun' ||
                              channelData.dates[4].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[5].day === 'Sun' ||
                              channelData.dates[5].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[6].day === 'Sun' ||
                              channelData.dates[6].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[7].day === 'Sun' ||
                              channelData.dates[7].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[8].day === 'Sun' ||
                              channelData.dates[8].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                    ...
                    render: (text: string) => {
                      return {
                        props: {
                          style: {
                            background:
                              channelData.dates[9].day === 'Sun' ||
                              channelData.dates[9].day === 'Sat'
                                ? 'rgb(255, 242, 232)'
                                : '',
                          },
                        },
                ...
                for (let index = 0; index < channelLength; index++) {
                  if (index === 0) {
                    ...
                    if (
                      ...
                      channelSearch.channel ===
                        item.standard_rates[index - 2]?.channel?.toLowerCase()
                    ) {
                      data.push({
                        key: '1',
                ...
                return (
                  <Col className={reservationListStyles.channelTableSection} span={24}>
                    <Table
                      bordered
                      className={reservationListStyles.channelTable}
                      columns={columns}
                      dataSource={data}
                      pagination={false}
                      rowClassName={(record: object, index: number) => {
                        if (index > 1) {
                          return reservationListStyles.channelRowGreen;
                        }
                    ...
                    {item.standard_rates.length > 5 && (
                      <div className={reservationListStyles.channelShowMoreWrap}>
                        <MButton
                          onClick={() => {
