/** ***********************************
Module Name : Monthly Invoice
Developer Name : ThuLT
Created Date : 20/10/2023
Updated Date : 20/10/2023
Main functions : Show detail monthly invoice
************************************ */

import React, { useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { Col, Form, Modal, Row } from 'antd';
import { getAPI } from 'helpers/apiService';
import { selectGetReservation, selectGetReservationDetail } from 'selectors';

import { useAppSelector } from 'modules/hooks';

interface Props {
  invoiceId?: string | number;
  isModalOpen: boolean;
  setModalVisible: (value: boolean) => void;
}

interface DataDetail {
  description: string;
  file_url: string;
  month: string;
  total: string;
}

function InvoiceDetailModal({ invoiceId, isModalOpen, setModalVisible }: Props) {
  const { t } = useTranslation();
  const [data, setData] = useState<DataDetail>();
  const reservationData = useAppSelector(selectGetReservation).data;
  const reservationDetailInfoData = useAppSelector(selectGetReservationDetail).data;

  useEffect(() => {
    async function getInvoiceDetail() {
      const item = await getAPI(`api/v1/monthly-invoices/${invoiceId}/detail`);

      setData(item?.data);
    }

    getInvoiceDetail();
  }, [invoiceId]);

  return (
    <Modal
      bodyStyle={{ backgroundColor: '#F0F2F5' }}
      cancelButtonProps={{ style: { borderRadius: 4 } }}
      onCancel={() => setModalVisible(false)}
      title={<b>{t('monthlyInvoice.Invoice Detail')}</b>}
      visible={isModalOpen}
      width={850}
    >
      <Form layout="vertical">
        <Row>
          <Col span={9} style={{ paddingRight: 15 }}>
            <Form.Item label={t('monthlyInvoice.Folio ID.title')}>
              <span style={{ fontWeight: 600 }}> {reservationData.reservation_number}</span>
            </Form.Item>
            <Form.Item label={t('monthlyInvoice.Type.title')} name="description_id">
              <span style={{ fontWeight: 600 }}> {data?.description}</span>
            </Form.Item>
          </Col>
          <Col span={9} style={{ paddingLeft: 15 }}>
            <Form.Item label={t('monthlyInvoice.Guest Name.title')}>
              <span style={{ fontWeight: 600 }}> {reservationDetailInfoData.guest_name} </span>
            </Form.Item>
            <Form.Item label={t('monthlyInvoice.Total.title')} name="actual_amount">
              <span style={{ fontWeight: 600 }}> {data?.total} </span>
            </Form.Item>
          </Col>
          <Col span={6}>
            <Form.Item
              label={t('monthlyInvoice.Use Month.title')}
              name="use_month"
              style={{ paddingLeft: 20 }}
            >
              <span style={{ fontWeight: 600 }}> {data?.month} </span>
            </Form.Item>
          </Col>
          {data?.file_url && (
            <>
              <Col span={1}>
                <span className="ant-upload-span">
                  <div className="ant-upload-text-icon">
                    <span aria-label="paper-clip" className="anticon anticon-paper-clip" role="img">
                      <svg
                        aria-hidden="true"
                        data-icon="paper-clip"
                        fill="currentColor"
                        focusable="false"
                        height="1em"
                        viewBox="64 64 896 896"
                        width="1em"
                      >
                        <path d="M779.3 196.6c-94.2-94.2-247.6-94.2-341.7 0l-261 260.8c-1.7 1.7-2.6 4-2.6 6.4s.9 4.7 2.6 6.4l36.9 36.9a9 9 0 0012.7 0l261-260.8c32.4-32.4 75.5-50.2 121.3-50.2s88.9 17.8 121.2 50.2c32.4 32.4 50.2 75.5 50.2 121.2 0 45.8-17.8 88.8-50.2 121.2l-266 265.9-43.1 43.1c-40.3 40.3-105.8 40.3-146.1 0-19.5-19.5-30.2-45.4-30.2-73s10.7-53.5 30.2-73l263.9-263.8c6.7-6.6 15.5-10.3 24.9-10.3h.1c9.4 0 18.1 3.7 24.7 10.3 6.7 6.7 10.3 15.5 10.3 24.9 0 9.3-3.7 18.1-10.3 24.7L372.4 653c-1.7 1.7-2.6 4-2.6 6.4s.9 4.7 2.6 6.4l36.9 36.9a9 9 0 0012.7 0l215.6-215.6c19.9-19.9 30.8-46.3 30.8-74.4s-11-54.6-30.8-74.4c-41.1-41.1-107.9-41-149 0L463 364 224.8 602.1A172.22 172.22 0 00174 724.8c0 46.3 18.1 89.8 50.8 122.5 33.9 33.8 78.3 50.7 122.7 50.7 44.4 0 88.8-16.9 122.6-50.7l309.2-309C824.8 492.7 850 432 850 367.5c.1-64.6-25.1-125.3-70.7-170.9z" />
                      </svg>
                    </span>
                  </div>
                </span>
              </Col>
              <Col span={23}>
                <a href={data?.file_url} rel="noreferrer" target="_blank">
                  {data?.file_url}
                </a>
              </Col>
            </>
          )}
        </Row>
      </Form>
    </Modal>
  );
}

export default InvoiceDetailModal;
